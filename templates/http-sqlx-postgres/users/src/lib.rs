//! The stateless `/users` backend.
//!
//! It exports `wasmcloud:app/users` and is invoked by the Service's router
...
mod bindings {
    #![allow(unsafe_code)]
    wit_bindgen::generate!({ path: "../wit", world: "users-backend" });
}

use std::sync::OnceLock;

use bindings::exports::wasmcloud::app::users::{Guest, Request, Response};
...
fn query() -> anyhow::Result<String> {
    ...
    rt.block_on(async {
        ...
        let items: Vec<serde_json::Value> = rows
            ...
            .collect();
        Ok::<_, anyhow::Error>(serde_json::json!({ "users": items }).to_string())
    })
}
