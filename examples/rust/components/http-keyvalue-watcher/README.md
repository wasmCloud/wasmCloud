...
You can set triggers for key-value operations using link configuration by adding a `watch` field with a comma-separated string of watch patterns. Each pattern follows the format `OPERATION@keypattern`
```bash
# wash config put <config-name> <values...>
wash config put custom-config foo=bar watch=SET@key:value,DEL@key
```
