...
use wasmcloud_component::wasi::keyvalue::{watcher, store};
...
use crate::bindings::wasi::http::types::OutgoingRequest;
...
impl KvWatcherDemoGuest for KvWatcher {
    fn on_set(bucket: bindings::wasi::keyvalue::store::Bucket, key: String, value: Vec<u8>) {
        ...
        req.set_path_with_query(Some(&format!("/alert?{}=has_been_set_to_{:?}",&key,String::from_utf8(value)).to_string()))
            .unwrap();
        let _ = outgoing_handler::handle(req, None);
    }
    fn on_delete(bucket: bindings::wasi::keyvalue::store::Bucket, key: String) {
        ...
        req.set_scheme(Some(&Scheme::Https)).unwrap();
        req.set_authority(Some("localhost:3001")).unwrap();
        req.set_path_with_query(Some(&format!("/alert?{}=nil_cuz_deleted",&key).to_string()))
            .unwrap();
        let _ = outgoing_handler::handle(req, None);
    }
}
...
impl HttpKvInvoker for KvWatcher {
    fn handle(request: IncomingRequest, response_out: ResponseOutparam) {
                        ...
                        match store::open(&link_name) {
                            ...
                            Err(e) => {
                                ...
                                error_body
                                    .write()
                                    .unwrap()
                                    .blocking_write_and_flush(format!("failed to open bucket {:?}", e).as_bytes())
                                    .unwrap();
                                OutgoingBody::finish(error_body, None).expect("failed to finish response body");
                                ResponseOutparam::set(response_out, Ok(error_response));
                                return;
        ...
        let message = match (action.as_deref(), value) {
            (Some("on_set"), Some(val)) => kv_watcher.on_set(Some(link_name.to_string()), key, val.into_bytes()),
...
impl KvWatcher {
    fn on_set(&mut self, link_name: Option<String>, key: String, value: Vec<u8>) -> Result<String, ErrorCode> {
