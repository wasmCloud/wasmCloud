...
async fn test_roundtrip() -> Result<()> {
    ...
    let JobSubmitApiResponse { job_id } = submit_image_for_processing(&env).await?;
    assert!(!job_id.is_empty(), "job_id was unexpectedly missing");

    let started = Instant::now();
    loop {
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Submit an image for processing
        let JobStatusResponse { status } = check_processing_status(&env).await?;
        if status == "completed" {
            break;
        }

        // Wait at max 5 seconds to finish processing
        if Instant::now().duration_since(started) > Duration::from_secs(5)
            bail!("time elapsed waiting for processing, last status [{status}]");
        }
    }
