...
#![allow(clippy::expect_used, unreachable_code)]

use crate::{cargo_cmd, show_cmd};
...
fn run_cargo_build_wasm(
    ...
    match status {
        ...
        Ok(_) => {
            tracing::warn!("failed to build {fixture}");
            false
        }
        Err(e) => {
            tracing::warn!("Failed to execute cargo for {fixture}: {e}");
            false
        }
    }
}
...
fn build_fixtures(
    ...
    for fixture in fixtures {
        ...
        if !run_cargo_build_wasm(&fixtures_dir, fixture, target, &target_dir) {
        ...
        let Some(wasm_path) = wasm_path else {
            tracing::warn!("No artifact for fixture {fixture}");
            continue;
        };
    ...
    if !failures.is_empty() {
        anyhow::bail!("failed to build fixtures: {failures:?}")
    }
