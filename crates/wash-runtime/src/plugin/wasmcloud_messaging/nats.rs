...
impl HostPlugin for NatsMessaging {
    ...
    async fn on_workload_resolved(
        ...
        // A long-lived handler service has no per-component instance
        // to pre-instantiate — its receive loop delivers to the running service
                    ...
                    maybe_msg = messages.next() => {
                            ...
                            .await
                        {
                            ...
                            match workload
                                ...
                                .await
                            {
                                Ok(Ok(())) => debug!(%subject, "reactor handled message"),
                                Ok(Err(e)) => {
                                    warn!(%subject, error = %e, "reactor message handler returned error")
                                }
                                Err(e) => {
                                    warn!(%subject, error = %e, "failed to deliver message to reactor")
                                }
                            }
                            continue;
                        }
...
mod tests {
    ...
    //! NATS / wasmtime: between the plugin and its tracker, and
