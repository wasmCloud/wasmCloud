...
/// workload its calls go to. Versioned at the package's first release: wasmtime
/// resolves a plugin's import of a later `wasmcloud:host` patch against this
/// definition, so the constant does not move when the package version does.
pub(super) const HOST_WORKLOAD_INTERFACE: &str = "wasmcloud:host/workload@0.1.0";
