    ...
    fn grant_tls(
        ...
        for server in &config.servers {
            let addr = server
                .parse::<async_nats::ServerAddr>()
                .with_context(|| format!("NATS server {server:?} is invalid"))?;
            // `nats://` and `tls://` are one protocol, which negotiates TLS
            // itself; an entry for any other protocol on this port is not.
            let schemes: &[&str] = if addr.is_websocket() {
                &["ws", "wss"]
            } else {
                &["nats", "tls"]
            };
...
mod tests {
    ...
    /// `nats://` and `tls://` are one protocol; a grant for another protocol on
    /// the same port does not reach NATS.
    #[test]
    fn a_grant_applies_to_the_nats_schemes_only() {
        let plugin = with_grant_tls("tls://nats.internal:4222");
        for server in ["nats://nats.internal:4222", "tls://nats.internal:4222"] {
            assert!(
                plugin.grant_tls(&nats_config(server)).unwrap().is_some(),
                "{server}"
            );
        }
        let plugin = with_grant_tls("https://broker.internal:8443");
        assert!(
            plugin
                .grant_tls(&nats_config("nats://broker.internal:8443"))
                .unwrap()
                .is_none()
        );
    }
