...
async fn new_ephemeral_store(
    ...
    // One read-lock scope, no `WorkloadMetadata` clones: cloning metadata
    // deep-clones its by-value `Linker` (~50µs each — perf notebook H7), and
    // this ran per ephemeral call for the active component plus every linked
    // one. Ctx templates copy only the cheap fields. `pre_instantiate_ref`
    // needs only `&Linker`, so no write lock (which would serialize every
    // concurrent ephemeral call) is required either.
    #[cfg(feature = "wasi-tls")]
