...
use crate::host::quota::GuestConnectionQuota;
...
pub async fn publish(
    ...
    if request.protocol != Protocol::Tcp {
        bail!(
            "{} declares published port '{}' as {}, but only TCP ports can be published",
            request.owner,
            request.name,
            request.protocol
        );
    }
    publish_tcp(config, table, request).await
}
...
async fn accept_loop(
    ...
    loop {
        ...
        let inbound = match quota.as_ref() {
            Some(quota) => match quota.try_acquire_inbound_socket() {
                Some(slot) => Some(slot),
                None => {
                    debug!(
                        %owner,
...
async fn splice(stream: TcpStream, conn: loopback::TcpConn) -> Result<()> {
    ...
    let inbound = async move {
        ...
        loop {
            ...
            // One permit covers one chunk however large it is, so the read is
            // capped: `MAX_INFLIGHT_CHUNKS * CHUNK_SIZE` is only a real memory
            // bound while no chunk exceeds `CHUNK_SIZE`. `reserve` may hand
            // back more spare capacity than asked for.
            let n = read_half
                .read_buf(&mut (&mut buf).limit(CHUNK_SIZE))
                .await?;
            if n == 0 {
                // The peer half-closed. Dropping `to_guest` is what the guest
                // observes as end-of-input; its reply is still on the way, so
                // this direction ends without ending the other.
                drop(to_guest);
                return Ok(());
            }
            // Hands the guest the bytes just read without copying them: the
            // `Bytes` shares this allocation and releases its share when the
            // guest is done, which is also when the permit returns.
    ...
    // Both directions run to completion. Ending the pair on whichever finishes
    // first would discard the guest's reply whenever a peer half-closes its
    // write side and waits for an answer — the ordinary shape of a
    // FIN-delimited request.
...
mod tests {
    ...
    async fn a_spliced_connection_round_trips_and_carries_the_real_peer() {
        ...
        guest_tx
            .send((bytes::Bytes::from_static(b"pong"), permit))
            .unwrap();
