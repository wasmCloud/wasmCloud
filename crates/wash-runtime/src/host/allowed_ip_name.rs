...
//! [`AllowedIpName`], and [`AllowedIpName::matches`] evaluates a requested name
...
//! | ----------------- | ------------------------------- |
//! | `*`               | [`AllowedIpName::Any`]            |
//! | `*.example.com`   | [`AllowedIpName::SuffixWildcard`] |
//! | `example.com`     | [`AllowedIpName::Exact`]          |
//! | `127.0.0.1`, `::1`| [`AllowedIpName::Ip`]             |
//!
...
//! - [`AllowedIpName::SuffixWildcard`] requires a non-empty prefix, so
//!   `example.com` does not satisfy `*.example.com`.
//! - [`AllowedIpName::Ip`] compares parsed addresses, so `::1` and
//!   `0:0:0:0:0:0:0:1` are the same entry.
//! - Only [`AllowedIpName::Any`] and [`AllowedIpName::Ip`] match a literal
...
//!
//! let policy: AllowedIpName = "*.example.com".parse().unwrap();
...
impl AllowedIpName {
    ...
    pub fn matches(&self, host: &url::Host<String>) -> bool {
        match (self, host) {
            (AllowedIpName::Any, _) => true,
            (AllowedIpName::Ip(allowed), url::Host::Ipv4(addr)) => *allowed == IpAddr::V4(*addr),
            (AllowedIpName::Ip(allowed), url::Host::Ipv6(addr)) => *allowed == IpAddr::V6(*addr),
            (AllowedIpName::Ip(_), url::Host::Domain(_)) => false,
            (AllowedIpName::Exact(allowed), url::Host::Domain(domain)) => {
                strip_root_dot(domain).eq_ignore_ascii_case(allowed)
            }
            (AllowedIpName::SuffixWildcard { suffix }, url::Host::Domain(domain)) => {
                let domain = strip_root_dot(domain);
                domain.len() > suffix.len()
                    && domain[domain.len() - suffix.len()..].eq_ignore_ascii_case(suffix)
            }
            (AllowedIpName::Exact(_) | AllowedIpName::SuffixWildcard { .. }, _) => false,
        }
    }
}
...
#[must_use]
pub fn check_allowed_ip_name(policy: &[AllowedIpName], host: &url::Host<String>) -> bool {
...
impl FromStr for AllowedIpName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(suffix) = trimmed.strip_prefix("*.") {
            ...
            return Ok(AllowedIpName::SuffixWildcard {
        ...
        if let Ok(addr) = trimmed.parse::<IpAddr>() {
            return Ok(AllowedIpName::Ip(addr));
        }
        if trimmed.contains(':') {
            return Err(anyhow!(
        ...
        Ok(AllowedIpName::Exact(trimmed.to_ascii_lowercase()))
    }
}

impl fmt::Display for AllowedIpName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AllowedIpName::Any => f.write_str("*"),
            AllowedIpName::Exact(name) => f.write_str(name),
            // `suffix` already carries the leading dot.
            AllowedIpName::SuffixWildcard { suffix } => write!(f, "*{suffix}"),
            AllowedIpName::Ip(addr) => write!(f, "{addr}"),
        }
    }
}

impl Serialize for AllowedIpName {
    fn serialize<S: ser::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        serializer.collect_str(self)
    }
}

impl<'de> Deserialize<'de> for AllowedIpName {
...
mod tests {
    ...
    fn empty_policy_denies_everything() {
        assert!(!check_allowed_ip_name(&[], &host("example.com")));
        assert!(!check_allowed_ip_name(&[], &host("127.0.0.1")));
    }

    #[test]
    fn check_matches_any_entry() {
        let policy = [parse("example.com"), parse("*.internal")];
        assert!(check_allowed_ip_name(&policy, &host("example.com")));
        assert!(check_allowed_ip_name(&policy, &host("db.internal")));
        assert!(!check_allowed_ip_name(&policy, &host("evil.com")));
    }

    #[test]
    fn bare_star_wildcard_is_rejected() {
        // `*com` would match every `.com`, which is never the intent.
        let err = "*com".parse::<AllowedIpName>().unwrap_err();
    ...
    fn url_shaped_entries_are_rejected() {
        for entry in [
            ...
            assert!(
                entry.parse::<AllowedIpName>().is_err(),
    ...
    fn port_entries_are_rejected() {
        let err = "example.com:8080".parse::<AllowedIpName>().unwrap_err();
