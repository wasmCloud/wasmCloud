...
async fn main() -> anyhow::Result<()> {
    // Honor RUST_LOG so the routing-policy fairness pass can tally per-replica
    // selection from debug logs; default (unset) stays log-silent.
    ...
    while let Some(a) = args.next() {
        ...
        match a.as_str() {
            ...
            "--service" => service_path = Some(value("--service")?),
            "--router" => router = value("--router")?,
    ...
    eprintln!(
        "bench host serving on http://{addr} (router={router}, host={hostname}, replicas={replicas})"
    );
