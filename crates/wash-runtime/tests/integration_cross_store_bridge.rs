...
async fn test_bridge_async_call_primitives() -> Result<()> {
    let host = "bridge-add";
    let (addr, h) = start_host_with_p3("127.0.0.1:0").await?;
    h.workload_start(bridge_workload(host)).await?;
    let client = reqwest::Client::new();

    let (result, expected) = call(&client, &addr, host, "/add").await?;
    assert_eq!(result, expected, "ops.add(40,2) should reach the service as 42");
    assert_eq!(result, 42);
    Ok(())
}
...
async fn test_bridge_future_relocation() -> Result<()> {
    ...
    assert_eq!(result, expected, "backend future<u64> should reach the service");
...
async fn test_bridge_stream_backpressure_large() -> Result<()> {
    ...
    assert_eq!(result, expected, "every byte of the large stream must arrive");
    Ok(())
}
...
async fn test_bridge_reader_drops_early() -> Result<()> {
    ...
    assert!(read > 0, "should have read a non-empty prefix before dropping");
    assert!(
        read < full,
