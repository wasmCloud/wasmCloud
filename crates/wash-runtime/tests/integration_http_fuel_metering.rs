...
//!
//! Fuel is metering here, not a limit: `FuelConsumptionMeter::observe` measures
//! a call by the fuel it burns. But a store built on a fuel-enabled engine
//! starts at *zero* fuel and instantiation runs guest code, so a path that
//! builds a store without priming it traps before the guest runs at all — and
//! the failure surfaces as an empty 500 with no guest logs, because the guest
//! never started. Every host the wasmCloud chart deploys runs with
//! `--enable-meters`, so this is the configuration that matters.
...
use common::{
    ...
    http_counter_host_interfaces, http_only_host_interfaces, start_host_with_meters,
};
