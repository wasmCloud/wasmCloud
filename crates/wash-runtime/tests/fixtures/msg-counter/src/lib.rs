...
//! `"{count}:{subject}"` as its error result (observed directly by the reactor
...
//! long-lived instance the reactor co-drives, not a fresh one per message.
...
impl RunGuest for Component {
    async fn run() -> Result<(), ()> {
        ...
        // Keep the service alive; the reactor co-drives this concurrently with
...
impl MsgGuest for Component {
    fn handle_message(msg: BrokerMessage) -> Result<(), String> {
        ...
        // Echo the running count + subject so the reactor spike can observe
