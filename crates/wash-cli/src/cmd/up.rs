...
use wash_lib::start::{
    ...
    NatsConfig, WadmConfig, NATS_SERVER_BINARY, NATS_SERVER_CONF, WADM_PID,
};
...
async fn install_patch_or_default_wadm_version(
    ...
    const WASMCLOUD_ORG: &str = "wasmCloud";
    const WASMCLOUD_WADM_REPO: &str = "wadm";
    let version = version.clone().unwrap_or(WADM_VERSION.to_owned());
    let new_patch_version =
        new_patch_version_of_after_string(WASMCLOUD_ORG, WASMCLOUD_WADM_REPO, &version).await?;
