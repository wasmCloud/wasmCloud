...
#[allow(unused)]
const TEMP_DIR_NAME: &str = "/tmp/wash_integration_test_temp";
...
pub async fn start_nats(port: u16, nats_install_dir: &PathBuf) -> Result<Child> {
    let nats_binary = ensure_nats_server(NATS_SERVER_VERSION, nats_install_dir).await?;
...
pub struct TestWashInstance {
    /// ID of the host
    pub host_id: String,
    /// Directory that acts as a temporary home dir for wash
...
impl Drop for TestWashInstance {
    fn drop(&mut self) {
        let TestWashInstance { kill_cmd, .. } = self;
...
impl TestWashInstance {
    ...
    async fn new(args: TestWashInstanceNewArgs) -> Result<Self> {
        ...
        ensure_downloads(&test_dir).await?;
        let nats_port = find_open_port().await?;
        let nats = start_nats(nats_port, &test_dir.path().join(".wash").join("downloads")).await?;
...
/// Helper function that copies all download artifacts into the given directory from a known temp
/// directory. Unfortunately, due to the way `cargo nextest` works (running the tests in separate
/// processes), we have to copy the artifacts in before running the tests. Initially this used a
/// `OnceCell` with a temp directory that we populated, but that just ends up doing nothing when
/// running with nextest. This does save a lot of time on tests though, so the brittleness is worth
/// it.
async fn ensure_downloads(dir: impl AsRef<Path>) -> Result<()> {
    let fake_home = PathBuf::from(TEMP_DIR_NAME);

    // Copy the artifacts from the downloads directory to the test directory.
    let downloads = fake_home.join(".wash").join("downloads");
    let output_downloads = dir.as_ref().join(".wash").join("downloads");
    tokio::fs::create_dir_all(&output_downloads)
        .await
        .context("Unable to create output downloads directory")?;
    // Copy the various files, ignoring if they don't exist.
    match tokio::fs::copy(downloads.join("wadm"), output_downloads.join("wadm")).await {
        Ok(_) => {}
        Err(e) => {
            if e.kind() != std::io::ErrorKind::NotFound {
                return Err(e.into());
            } else {
                println!("wadm not found in downloads directory");
            }
        }
    }

    match tokio::fs::copy(
        downloads.join("nats-server"),
        output_downloads.join("nats-server"),
    )
    .await
    {
        Ok(_) => {}
        Err(e) => {
            if e.kind() != std::io::ErrorKind::NotFound {
                ...
                println!("nats-server not found in downloads directory");
            }
        }
    }

    // Iterate over the directory and find the first `v` prefixed directory and then copy that directory and its contents
    let mut readdir = match tokio::fs::read_dir(&downloads).await {
        Ok(dir) => dir,
        Err(e) => {
            if e.kind() != std::io::ErrorKind::NotFound {
                return Err(e.into());
    ...
    while let Some(entry) = readdir.next_entry().await? {
        ...
        if file_name.starts_with('v') {
            tokio::fs::create_dir_all(output_downloads.join(file_name))
                .await
                .context("Unable to create downloads directory")?;
            tokio::fs::copy(
                entry_path.join("wasmcloud_host"),
                output_downloads.join(file_name).join("wasmcloud_host"),
            )
            .await
            .context("Unable to copy wasmcloud_host")?;
            break;
        }
    }
    Ok(())
...
impl TestSetup {
    ...
    async fn new(test_dir: TempDir, project_dir: PathBuf) -> Result<Self> {
        ...
        ensure_downloads(fake_home.path()).await?;
        Ok(Self {
            test_dir,
            project_dir,
            fake_home,
    ...
    pub fn base_command(&self) -> Command {
        ...
        cmd.env("HOME", self.fake_home.path());
        cmd
    }
}
