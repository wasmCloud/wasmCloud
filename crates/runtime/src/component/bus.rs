...
impl fmt::Debug for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Error::LinkNotFound(error) | Error::Transport(error) => error.fmt(f),
        }
    }
}
...
impl<H: Handler> error::HostError for Ctx<H> {
    ...
    async fn from_rpc_error(
    ...
    ) -> wasmtime::Result<Result<Resource<Error>, Resource<rpc::Error>>> {
        let rpc::Error(error) = self
            .table
            .delete(error)
        ...
        match error.downcast() {
            ...
            Err(error) => {
                let error = self
                    .table
                    .push(rpc::Error(error))
                    .context("failed to push `wrpc:rpc/error.error` to table")?;
                Ok(Err(error))
            }
        }
    }
