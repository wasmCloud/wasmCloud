...
///
/// # Arguments
/// * `map` - HashMap containing string representations of limits
///
/// # Returns
/// Limits struct with parsed values or None for invalid/missing entries
pub fn from_string_map(map: &HashMap<String, String>) -> Limits {
    Limits {
    ...
    H: Handler,
{
    ...
    pub fn new_with_linker(
        ...
        let engine: wasmtime::Engine = if limits.unwrap().max_memory_limit.is_none() {
            rt.engine.clone()
        } else {
            ...
            component_pooling_config
                .max_memory_size(limits.clone().unwrap().max_memory_limit.unwrap());
