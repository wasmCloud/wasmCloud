...
use wasmcloud_provider_sdk::{core::LinkDefinition, error::InvocationResult};
...
pub type AsyncCallActorFn = Box<
    dyn Fn(
            ...
            Request,
            Option<Duration>,
        ) -> Pin<Box<dyn Future<Output = InvocationResult<Response>> + Send + 'static>>
        + Send
        + Sync,
>;
...
impl HttpServerCore {
    ...
    where
        F: Fn(String, Arc<LinkDefinition>, Request, Option<Duration>) -> Fut
            ...
            + Sync
            + 'static,
        Fut: Future<Output = InvocationResult<Response>> + 'static + Send,
    {
