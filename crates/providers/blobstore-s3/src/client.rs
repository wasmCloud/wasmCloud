...
use wasmcloud_provider_wit_bindgen::deps::wasmcloud_provider_sdk::{
    Context, InterfaceLinkDefinition,
};
...
pub struct StorageClient {
    s3_client: S3Client,
    ld: Arc<InterfaceLinkDefinition>,
...
impl StorageClient {
    pub async fn new(config: StorageConfig, ld: InterfaceLinkDefinition) -> Self {
    ...
    pub async fn close(&self) {
        debug!(source_id = %self.ld.source_id, "blobstore-s3 dropping linkdef");
    ...
    pub async fn send_chunk(&self, ctx: &Context, mut chunk: Chunk) -> u64 {
        ...
        let source_id = &self.ld.source_id;
