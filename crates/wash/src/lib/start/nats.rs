...
/// let res = ensure_nats_server("v2.11.3", "/tmp/").await;
/// assert!(res.is_ok());
...
/// let res = ensure_nats_server_for_os_arch_pair(os, arch, "v2.11.3", "/tmp/").await;
/// assert!(res.is_ok());
/// assert!(res.unwrap().to_string_lossy() == "/tmp/nats-server");
/// # }
/// ```
...
/// let res = download_nats_server("v2.11.3", "/tmp/").await;
/// assert!(res.is_ok());
/// assert!(res.unwrap().to_string_lossy() == "/tmp/nats-server");
/// # }
/// ```
