...
impl WasmCloudTestHost {
    ...
    pub async fn start(
        ...
        lattice_names: impl Into<Arc<[Box<str>]>>,
    ) -> Result<Self> {
    ...
    pub async fn get_ctl_client(
        ...
        if !self.lattice_names.contains(&Box::from(lattice)) {
    ...
    #[must_use]
    pub fn lattices(&self) -> Arc<[Box<str>]> {
