...
//!     fn handle(_request: http::IncomingRequest) -> Result<http::Response<&'static str>, http::ErrorCode> {
...
impl crate::From<HeaderMap> for wasi::http::types::Fields {
...
impl OutgoingBody for wasi::http::types::IncomingBody {
    fn write(
        ...
        drop(stream);
        let trailers = wasi::http::types::IncomingBody::finish(self);
...
impl From<IncomingBody> for InputStream {
    fn from(IncomingBody { stream, .. }: IncomingBody) -> Self {
        stream
    }
}
...
impl IncomingBody {
    ...
    pub fn into_trailers_wasi(self) -> anyhow::Result<Option<wasi::http::types::Fields>> {
        ...
        drop(stream);
        let trailers = wasi::http::types::IncomingBody::finish(body);
        let trailers = if let Some(trailers) = trailers.get() {
            trailers
        } else {
            trailers.subscribe().block();
        ...
        match crate::TryInto::try_into(request) {
            Ok(request) => match T::handle(request) {
                Ok(response) => {
                    ...
                    let headers = crate::Into::into(headers);
...
macro_rules! export {
    ($t:ty) => {
        ...
        wasi::http::proxy::export!(__IncomingHandlerExport);
    };
}
pub use export;
