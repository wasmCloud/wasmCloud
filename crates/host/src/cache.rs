    ...
    V: std::fmt::Debug + Clone + Send + Sync + 'static,
{
    ...
    fn get(&mut self, key: &K) -> Option<V> {
        match self.data.entry(key.clone()) {
            std::collections::hash_map::Entry::Occupied(entry) if entry.get().0.created_at.elapsed() < self.ttl => {
...
mod tests {
    ...
    async fn test_expiration_with_purge_cycles() {
        ...
        cache.insert("key2".to_string(), "value2".to_string()).await;
        assert_eq!(cache.get(&"key2".to_string()).await, Some("value2".to_string()));
        // The should be one because the key1 is expired and purge cycle was triggered
        assert_eq!(cache.internal.read().await.data.len(), 1);
    ...
    async fn test_purge_cycles_counter() {
        ...
        cache.insert("key1".to_string(), "value1".to_string()).await;
        assert!(cache.purge_cycle_count.load(Ordering::Relaxed) == 1);
        
        cache.get(&"key1".to_string()).await;
        assert!(cache.purge_cycle_count.load(Ordering::Relaxed) == 1);
        
        cache.insert("key2".to_string(), "value2".to_string()).await;
        assert!(cache.purge_cycle_count.load(Ordering::Relaxed) == 2);

        cache.poll_purge_once().await;
        assert!(cache.purge_cycle_count.load(Ordering::Relaxed) == 0);
