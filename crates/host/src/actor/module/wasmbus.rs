use crate::capability::host::Host;
...
impl Ctx {
    pub(super) fn new(handler: impl Into<Handler>) -> Self {
        Self {
...
async fn host_call(
    ...
    match trace_span!("Handle::handle")
        .in_scope(|| store.data_mut().wasmbus.handler.call(bd, ns, op, Some(pld)))
        .await
    {
        Ok(buf) => {
            set_host_response(&mut store, buf.unwrap_or_default().unwrap_or_default());
            Ok(wasm::SUCCESS)
        }
        Err(err) => {
