...
impl WorkloadIdentityConfig {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        let spiffe_endpoint = get_default_socket_path()
            .context("failed to load SPIFFE workload endpoint from SPIFFE_ENDPOINT_SOCKET")?;

        validate_socket_path(&spiffe_endpoint)
            .context("failed to validate SPIFFE workload endpoint from SPIFFE_ENDPOINT_SOCKET")?;

        let endpoint_uri = url::Url::parse(&spiffe_endpoint)?;
        ...
        if endpoint_uri.scheme() == UNIX_SCHEME && !Path::new(endpoint_uri.path()).exists() {
            bail!("provided SPIFFE workload endpoint does not exist: {spiffe_endpoint}");
        }
