...
async fn invoke_on_set(wrpc: &WrpcClient, bucket: &str, key: &str, value: &Bytes) {
    ...
    match bindings::wrpc::keyvalue::watcher::on_set(wrpc, Some(cx), bucket, key, value).await {
        Ok(_) => {
            info!(
                "Successfully invoked on_set for key '{}' from bucket '{}'",
                key, bucket
            );
        }
        Err(err) => {
            error!(
                "Failed to invoke on_set for bucket '{}' and key '{}': {}",
                bucket, key, err
            );
...
async fn invoke_on_delete(wrpc: &WrpcClient, bucket: &str, key: &str) {
    ...
    match bindings::wrpc::keyvalue::watcher::on_delete(wrpc, Some(cx), bucket, key).await {
        Ok(_) => {
            info!(
                "Successfully invoked on_delete key '{}' from bucket '{}'",
                key, bucket
            );
        }
        Err(err) => {
            error!(
                "Failed to invoke on_delete for bucket '{}' and key '{}': {}",
                bucket, key, err
            );
...
impl keyvalue::watcher::Handler<Option<Context>> for KvRedisProvider {
    async fn on_set(
        ...
        info!(
            "phase 1 : watching key : {} for set of value : {}",
            key, value_str
        );
        Ok(())
    }
    ...
    async fn on_delete(
        ...
        info!("phase 1 : watching key : {} for deletion", key);
        Ok(())
    }
}
...
impl Provider for KvRedisProvider {
    ...
    async fn receive_link_config_as_source(
        ...
        if interfaces.contains(&"watcher".to_string()) {
            ...
            info!(
                "{}",
                "interface contains watcher, enabling key-space-events"
            );
            ...
            let task = tokio::spawn(async move {
                ...
                while let Some(msg) = stream.next().await {
                    info!("{}", "Notification recieved : unwrapping it now");
                    ...
                    info!("{}", format!("event : {} & channel : {}", event, channel));
                    ...
                    if let Some(key_info_set) = watched_keys.get(mkey.as_str()) {
                        if event == "set" {
                                ...
                                .await
                            {
                                Ok(Some(v)) => v.into(),
                                Ok(None) => {
                                    info!("Key {} not found or was deleted", mkey);
                                    continue;
                                }
                                Err(e) => {
