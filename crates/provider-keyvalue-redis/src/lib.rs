        ...
        if interfaces.contains(&"watcher".to_string()) {
            ...
            if let Some(watch_config) = config.get(&UniCase::new("watch")) {
                for watch_entry in watch_config.split(',') {
                    ...
                    if parts.len() != 2 {
                        error!("Invalid watch entry format: {watch_entry}. Expected FORMAT@KEY[:VALUE]");
                    }
                    ...
                    match operation.as_str() {
                        "SET" | "set" => {
                            ...
                            if key_value_parts.len() != 2 {
                                error!("Invalid SET watch format: {key_value}. SET requires both KEY:VALUE");
                            }
                            watched_keys
                                .entry(key_value_parts[0].to_string())
                                .or_default()
                                .insert(WatchedKeyInfo {
                                    event_type: WatchEventType::Set,
                                    value: Some(key_value_parts[1].to_string()),
                        ...
                        "DEL" | "del" => {
                            if key_value.contains(':') {
                                error!(
                                    "Invalid DEL watch format: {key_value}. DEL expects only KEY"
                                );
                            }
                            watched_keys
                        ...
                        _ => {
                            error!("Unsupported watch operation: {operation}. Expected SET or DEL")
                        }
                    }
                }
            }
