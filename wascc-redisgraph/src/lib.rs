//! # waSCC Graph Database Capability Provider: RedisGraph
//! 
...
impl WasccRedisgraphProvider {
    ...
    fn delete_graph(&self, actor: &str, delete: DeleteRequest) -> Result<Vec<u8>, Box<dyn Error>> {
        ...
        match rs {
            Ok(_) => Ok(vec![]),
            Err(e) => {
                Err(format!("Failed to delete graph: {:?}", e).into())
            }
        }
    }
    ...
    // We create a Redis client in responds to this message
