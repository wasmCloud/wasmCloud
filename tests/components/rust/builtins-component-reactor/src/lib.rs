...
use wasmcloud_component::wasi::{blobstore, keyvalue};
use wasmcloud_component::wasmcloud::{bus, messaging};
...
impl exports::wasi::http::incoming_handler::Guest for Actor {
    fn handle(request: http::types::IncomingRequest, response_out: http::types::ResponseOutparam) {
        ...
        //assert!(test_components::foobar::foobar::foobar("foo").is_err());
        ...
        assert_eq!(test_components::foobar::foobar::foobar("foo"), "foobar");
