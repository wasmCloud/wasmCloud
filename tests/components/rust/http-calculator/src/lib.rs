use crate::wasmcloud::calculator::calculator;
use crate::wasmcloud::calculator::types::MathOperator;
...
wit_bindgen::generate!({ generate_all, generate_unused_types: true });
...
impl http::Server for Component {
    fn handle(
        ...
        let op_req = calculator::OperationRequest {
            num1: 20,
            num2: 25,
            operator: MathOperator::Add,
        };

        let result = calculator::calculate(op_req)
            .map_err(|e| ErrorCode::InternalError(Some(format!("failed to calculate {e:?}"))))?;
