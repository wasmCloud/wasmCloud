...
use test_components::RUST_KEYVALUE_WATCHER;
...
const COMPONENT_ID: &str = "keyvalue_watcher";
...
async fn example_rust_keyvalue_watch() -> anyhow::Result<()> {
    ...
    try_join!(
        ...
        async {
            assert_scale_component(
                ...
                format!("file://{RUST_KEYVALUE_WATCHER}"),
                COMPONENT_ID,
                None,
                5,
                Vec::new(),
    ...
    assert_eq!(
        ...
        "foo: Successsfully created on_set trigger\n"
    );

    assert_eq!(
        assert_watch(&http_client, http_port, "on_delete", "foo", None).await?,
        "foo Successsfully created on_delete trigger\n"
    );
